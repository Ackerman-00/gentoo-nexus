...
if [[ -f "$RELAY" ]]; then
  ...
  if [[ "$dep_rows" -lt 18 ]]; then
    echo "❌ NOT COMPLETE — dependency audit table has $dep_rows rows, need 18"
    FAIL=1
  else
  ...
  if ! grep -qi "DOCKER BATTLE TEST\|docker.*stage3\|ldd.*not found" "$RELAY"; then
    echo "⚠️  WARNING: relay missing Docker battle test evidence (stage3 + ldd)"
  fi
else
...
for eb in */*/*.ebuild; do
  [[ -f "$eb" ]] || continue
  if ! grep -q "^EAPI=" "$eb" 2>/dev/null; then echo "❌ $eb missing EAPI"; bad=$((bad+1)); fi
done
if [[ "$bad" -gt 0 ]]; then echo "❌ NOT COMPLETE — $bad ebuilds malformed"; FAIL=1; fi
if [[ "$FAIL" -ne 0 ]]; then echo "❌ NOT COMPLETE — agent must continue working"; exit 1; fi
echo "✅ VERIFICATION PASSED — all 18 deps rows, evidence, install+battle test present"
exit 0
